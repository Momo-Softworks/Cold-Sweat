...
public class SoulSpringLampRenderer extends BlockEntityWithoutLevelRenderer
{
    ...
    public static final ResourceLocation TEXTURE_0 = ResourceLocation.fromNamespaceAndPath(ColdSweat.MOD_ID, "textures/item/soulspring_lamp/render/soulspring_lamp_0.png");
    public static final ResourceLocation TEXTURE_1 = ResourceLocation.fromNamespaceAndPath(ColdSweat.MOD_ID, "textures/item/soulspring_lamp/render/soulspring_lamp_1.png");
    public static final ResourceLocation TEXTURE_2 = ResourceLocation.fromNamespaceAndPath(ColdSweat.MOD_ID, "textures/item/soulspring_lamp/render/soulspring_lamp_2.png");
    ...
    public void renderByItem(ItemStack stack, ItemDisplayContext displayContext, PoseStack poseStack, MultiBufferSource buffer, int light, int overlay)
    {
        if (stack.is(ModItems.SOULSPRING_LAMP))
        {
            ...
            this.heart.render(poseStack, heartVtx, light, overlay, FastColor.ARGB32.colorFromFloat(1-emission, 1, 1, 1));
            // Emission
            VertexConsumer heartVtxEmissive = buffer.getBuffer(RenderType.entityTranslucentEmissive(texture));
            this.heart.render(poseStack, heartVtxEmissive, light, overlay, FastColor.ARGB32.colorFromFloat(emission, 1, 1, 1));
            poseStack.popPose();

            // Render glass
            VertexConsumer glassVtx = buffer.getBuffer(RenderType.entityTranslucent(texture));
            this.base.render(poseStack, glassVtx, light, overlay, FastColor.ARGB32.colorFromFloat(1-emission, 1, 1, 1));
            // Emission
            VertexConsumer glassVtxEmissive = buffer.getBuffer(RenderType.entityTranslucentEmissive(texture));
            this.base.render(poseStack, glassVtxEmissive, light, overlay, FastColor.ARGB32.colorFromFloat(emission, 1, 1, 1));
            poseStack.popPose();
        }
        else
        {
    ...
    private static ResourceLocation getTexture(ItemStack lamp)
    {
        ...
        int state;
        if (SoulspringLampItem.isLit(lamp))
        {
            state = fuel > 43 ? 3 :
                    fuel > 22 ? 2 : 1;
        }
