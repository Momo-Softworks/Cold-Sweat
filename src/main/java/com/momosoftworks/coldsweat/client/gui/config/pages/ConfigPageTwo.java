...
public class ConfigPageTwo extends AbstractConfigPage
{
    ...
    protected void init()
    {
        ...
        this.addSliderButton("water_droplet_scale", Side.LEFT,
                             () -> getSliderText(Component.translatable("cold_sweat.config.water_droplet_scale.name"), ConfigSettings.WATER_DROPLET_SCALE.get().min(), 5, 100, -1),
                             5, 100,
        ...
        // Direction Buttons: World Temp Gauge
        this.addDirectionPanel("food_effects_directions", Side.RIGHT, Component.translatable("cold_sweat.config.food_effects_location.name"),
                amount -> ConfigSettings.FOOD_EFFECTS_POS.set(new Vector2i(ConfigSettings.FOOD_EFFECTS_POS.get().x() + amount * ConfigScreen.SHIFT_AMOUNT.get(),
                                                                          ConfigSettings.FOOD_EFFECTS_POS.get().y())),
                amount -> ConfigSettings.FOOD_EFFECTS_POS.set(new Vector2i(ConfigSettings.FOOD_EFFECTS_POS.get().x(),
                                                                          ConfigSettings.FOOD_EFFECTS_POS.get().y() + amount * ConfigScreen.SHIFT_AMOUNT.get())),
                () ->
                {   ConfigSettings.FOOD_EFFECTS_POS.set(new Vector2i(0, 0));
                },
                () ->
                ...
                false, false, true, true, Component.translatable("cold_sweat.config.food_effects_location.desc"),
                                          Component.translatable("cold_sweat.config.offset_shift.name").withStyle(ChatFormatting.GRAY));
