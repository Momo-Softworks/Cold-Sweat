...
public class ConfigPageDifficulty extends Screen
{
    ...
    private static List<TextComponent> generateDescription(ConfigSettings.Difficulty difficulty)
    {
        return Arrays.asList(
                new TranslationTextComponent("cold_sweat.config.difficulty.description.min_temp", getTemperatureString(difficulty.getSetting(ColdSweat.createKey("min_temp")), BLUE)),
                new TranslationTextComponent("cold_sweat.config.difficulty.description.max_temp", getTemperatureString(difficulty.getSetting(ColdSweat.createKey("max_temp")), RED)),
                ...
                new StringTextComponent(new TranslationTextComponent("cold_sweat.config.difficulty.description.world_temp_"
                                                                  + (difficulty.getSetting(ColdSweat.createKey("require_thermometer")) ? "off" : "on"), BOLD + U_LINE, CLEAR).getString()),
                new StringTextComponent(new TranslationTextComponent("cold_sweat.config.difficulty.description.potions_"
                                                                  + (difficulty.getSetting(ColdSweat.createKey("ice_resistance_enabled")) ? "on" : "off"), BOLD + U_LINE, CLEAR).getString()));
    }
    ...
    public void render(@Nonnull MatrixStack matrixStack, int mouseX, int mouseY, float partialTicks)
    {
        ...
        // Draw Slider Head
        this.blit(matrixStack, this.width / 2 - 78 + (difficulty.ordinal() * 37), this.height / 2 - 58,
