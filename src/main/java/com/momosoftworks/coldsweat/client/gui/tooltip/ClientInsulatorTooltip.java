...
public class ClientInsulatorTooltip implements ClientTooltipComponent
{
    ...
    public ClientInsulatorTooltip(Pair<Double, Double> insulationValues, InsulationType type)
    {
    ...
    public void renderImage(Font font, int x, int y, GuiGraphics graphics)
    {
        ...
        if (posSlots > 0)
        {   renderBar(graphics, x, y, posSlots, cold, neutral, hot, isAdaptive, negSlots > 0, false, this.type == InsulationType.CURIO);
        ...
        if (negSlots > 0)
        {   renderBar(graphics, x, y, negSlots, -cold, -neutral, -hot, isAdaptive, true, true, this.type == InsulationType.CURIO);
            width += negSlots * 6 + 12;
        }
        poseStack.popPose();
    }
    ...
    static void renderBar(GuiGraphics graphics, int x, int y, int slots, double cold, double neutral, double hot, boolean isAdaptive, boolean showSign, boolean isNegative, boolean isCurio)
    {
        int coldSlots = Math.abs(CSMath.ceil(cold/2));
        int neutralSlots = Math.abs(CSMath.ceil(neutral/2));
        int hotSlots = Math.abs(CSMath.ceil(hot/2));
        ...
        // slots
        if (isAdaptive)
        {   renderCells(graphics, x + 7, y + 1, coldSlots, cold, 12, true); // adaptive cells
        }
        else
        {
            int xOffs = 0;
            if (cold > 0)
            {   renderCells(graphics, x + 7, y + 1, coldSlots, cold, 12, false); // cold cells
                xOffs += coldSlots * 6;
            }
            if (neutral > 0)
            {   renderCells(graphics, x + 7 + xOffs, y + 1, neutralSlots, neutral, 6, false); // neutral cells
                xOffs += neutralSlots * 6;
            }
            if (hot > 0)
            {   renderCells(graphics, x + 7 + xOffs, y + 1, hotSlots, hot, 18, false); // hot cells
            }
        }
        ...
        // icon
        if (isCurio)
        {   graphics.blit(TOOLTIP_LOCATION.get(), x, y - 1, 0, 24, 16, 8, 8, 32, 24);
        }
        else
