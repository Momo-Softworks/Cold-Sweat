...
public abstract class AbstractConfigPage extends Screen
{
    ...
    public AbstractConfigPage(Screen parentScreen, ITextComponent title)
    {   super(title);
        this.parentScreen = parentScreen;
    }
     ...
     */
    protected void addLabel(String id, Side side, IFormattableTextComponent text)
    {
                                   ...
                                   IFormattableTextComponent... tooltip)
    {
        ...
        // Make the input
        TextFieldWidget textBox = new TextFieldWidget(this.font, widgetX + labelOffset, widgetY - 6, boxWidth, 18, new StringTextComponent(""))
        {
            public void onEdit()
            {
                CSMath.tryCatch(() ->
        ...
        // Make the label
        ConfigLabel configLabel = new ConfigLabel(id, label.withStyle(Style.EMPTY.withColor(Color.fromRgb(shouldBeActive ? 16777215 : 8421504))), widgetX - 95, widgetY);
        // Add the clientside indicator
        if (clientside)
     ...
     */
    protected void addDirectionPanel(String id, Side side, IFormattableTextComponent label, Consumer<Integer> leftRightPressed, Consumer<Integer> upDownPressed, Runnable reset, Supplier<Boolean> visible,
                                     boolean requireOP, boolean setsCustomDifficulty, boolean clientside, boolean canHide, IFormattableTextComponent... tooltip)
    {
        ...
        if (canHide)
        {
            ImageButton hideButton = new ImageButton(widgetX + 52 + labelOffset, widgetY + 2, 20, 10, 68, 10, 20, TEXTURE, button ->
            {
                ...
                setButtonImageX((ImageButton) button, visible.get() ? 68 : 88);
            });
            visible.get();
            setButtonImageX(hideButton, visible.get() ? 68 : 88);
            hideButton.active = shouldBeActive;
            widgetBatch.add(hideButton);
        }

        // Add the option text
        ConfigLabel configLabel = new ConfigLabel(id, label.withStyle(Style.EMPTY.withColor(Color.fromRgb(shouldBeActive ? 16777215 : 8421504))), widgetX - 79, widgetY);
        // Add the clientside indicator
        if (clientside)
                                   ...
                                   IFormattableTextComponent... tooltip)
    {
        ...
        // Make the input
        ConfigSliderButton sliderButton = new ConfigSliderButton(widgetX, widgetY, label, 0, 1, 0,
                                                                 button -> {},
    ...
    {
        ImageWidget icon = new ImageWidget(TEXTURE, x, y, 12, 12, 0, 144);
        this.addWidget(icon);
        String iconId = String.format("%s_client", id);
        this.setTooltip(iconId, Arrays.asList(new TranslationTextComponent("cold_sweat.config.clientside_warning")));
        this.addWidgetBatch(iconId, Arrays.asList(icon), true);
    }
