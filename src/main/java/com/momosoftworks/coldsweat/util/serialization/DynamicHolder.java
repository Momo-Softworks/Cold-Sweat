...
public class DynamicHolder<T> implements Supplier<T>
{
    ...
    public CompoundNBT encode(DynamicRegistries registryAccess)
    {
        ...
        try
        {
            ...
            DataResult<INBT> holder = this.codec.encodeStart(NBTDynamicOps.INSTANCE, this.get(registryAccess));
            INBT encoded = holder.result().orElseThrow(RuntimeException::new);
    ...
    public void decode(CompoundNBT tag, DynamicRegistries registryAccess)
    {
        ...
        try
        {
            INBT encoded = tag.get(this.name.toString());
            if (encoded == null)
            {   throw ColdSweat.LOGGER.throwing(new SerializationException("No value found for DynamicHolder with name " + this.name));
            }
            this.value = this.codec.parse(NBTDynamicOps.INSTANCE, encoded).result().orElseThrow(RuntimeException::new);
