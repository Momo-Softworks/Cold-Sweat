...
public class InsulationItemsGroup extends ItemGroup
{
    ...
    private static List<ItemStack> sort(Collection<Map.Entry<Item, InsulatorData>> items)
    {
        ...
        list.sort(Comparator.comparing(entry -> {
            if (!(entry.getKey() instanceof ArmorItem)) return "";
            ArmorItem armor = (ArmorItem) entry.getKey();
            String materialName = armor.getMaterial().getName();
            return materialName + (3 - MobEntity.getEquipmentSlotForItem(armor.getDefaultInstance()).getIndex());
        }));
        ...
        return event.getItems().stream().map(entry ->
        {
            ...
            CompoundNBT nbt = entry.getValue().item().flatMap(it -> it.nbt().tag(), CompoundNBT::merge, (a, b) -> {}).orElse(new CompoundNBT());
            if (!nbt.isEmpty())
