...
public class IceBreakingEvents
{
    ...
    public static void onIceBreak(BlockEvent.BreakEvent event)
    {
        ...
        Material belowState = ilevel.getBlockState(pos.below()).getMaterial();
    ...
    public static void iceHarvestCheck(PlayerEvent.HarvestCheck event)
    {
        ...
        if (isModifiableIceBlock(state))
        {   event.setCanHarvest(tool.canPerformAction(ToolActions.PICKAXE_DIG) && event.getEntity().getMainHandItem().isCorrectToolForDrops(state));
        }
    }
