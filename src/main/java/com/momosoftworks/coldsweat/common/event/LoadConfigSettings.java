...
public class LoadConfigSettings
{
    ...
    {   ConfigSettings.load();
        ...
        ModRegistries.INSULATOR.getEntries()
        .forEach(entry ->
                {
                    Insulator settings = entry.getValue();
                    Insulation insulation = settings.insulation;
                    ...
                    settings.itemTag.ifPresent(tag ->
                    {
                        ...
                        ForgeRegistries.ITEMS.tags().getTag(tag).stream().forEach(item ->
                        {
                            ...
                            {
                                case ITEM -> ConfigSettings.INSULATION_ITEMS.get().put(item, Pair.of(insulation.getCold(), insulation.getHeat()));
                                case ADAPTIVE -> ConfigSettings.ADAPTIVE_INSULATION_ITEMS.get().put(item, Pair.of(insulation.value(), insulation.adaptSpeed().orElse(0d)));
