...
public class ColdSweatCommand extends BaseCommand
{
    ...
    private int executeReloadConfigs(CommandContext<CommandSourceStack> context)
    {
        try
        {
            ...
            context.getSource().sendSuccess(new TranslatableComponent("commands.cold_sweat.reload.success"), true);
        }
        catch (Exception e)
        {
            context.getSource().sendFailure(new TranslatableComponent("commands.cold_sweat.reload.failure", e.getMessage()));
    ...
    private int executeDumpConfigs(CommandContext<CommandSourceStack> context)
    {
        try
        {
            ...
            Style fileLinkStyle = Style.EMPTY.withClickEvent(new ClickEvent(ClickEvent.Action.OPEN_FILE, dumpPath.toAbsolutePath().toString()))
                                             ...
                                             .withHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT, new TranslatableComponent("mco.notification.visitUrl.buttonText.default")));
            context.getSource().sendSuccess(new TranslatableComponent("commands.cold_sweat.dump.success", new TextComponent(dumpPath.toString()).setStyle(fileLinkStyle)), true);
        }
        catch (Exception e)
        {
            context.getSource().sendFailure(new TranslatableComponent("commands.cold_sweat.dump.failure", e.getMessage()));
