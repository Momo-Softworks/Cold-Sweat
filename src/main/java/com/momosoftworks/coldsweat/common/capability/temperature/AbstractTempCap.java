...
public class AbstractTempCap implements ITemperatureCap
{
    ...
    public void tickDummy(LivingEntity entity)
    {
        ...
        Temperature.apply(this.getTrait(Trait.CORE), entity, Temperature.Trait.CORE, this.getAllModifiers(Trait.CORE));
    ...
    private double modifyFromAttribute(LivingEntity entity, Temperature.Trait trait, double baseValue)
    {
        ...
        ModifiableAttributeInstance attribute = attributes.computeIfAbsent(trait, t -> EntityTempManager.getAttribute(trait, entity));
        double newValue;
        ...
        if (!DoubleMath.fuzzyEquals(newValue, baseValue, 0.001))
        {
            ...
            NBTHelper.getOrPutTag(entity, "Temperature", new CompoundNBT()).putDouble(trait.getSerializedName(), newValue);
        }
        // Return
        return newValue;
    }
