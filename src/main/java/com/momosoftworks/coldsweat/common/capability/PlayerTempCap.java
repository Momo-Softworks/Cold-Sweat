    ...
    public void tick(LivingEntity entity)
    {
        ...
        if (!player.isCreative() && !player.isSpectator())
        {
            if (player.tickCount % 40 == 0 && !hasGrace)
            {
                ...
                {   this.dealTempDamage(player, ConfigSettings.DAMAGE_SCALING.get() ? damageSources.source(ModDamageSources.HOT_SCALING)
                                                                                    : damageSources.source(ModDamageSources.HOT),
                                        (float) CSMath.blend(ConfigSettings.TEMP_DAMAGE.get(), 0, heatResistance, 0, 1));
                }
                else if (bodyTemp <= -100 && !(hasIceResist && ConfigSettings.ICE_RESISTANCE_ENABLED.get()))
                {   this.dealTempDamage(player, ConfigSettings.DAMAGE_SCALING.get() ? damageSources.source(ModDamageSources.COLD_SCALING)
                                                                                    : damageSources.source(ModDamageSources.COLD),
                                        (float) CSMath.blend(ConfigSettings.TEMP_DAMAGE.get(), 0, coldResistance, 0, 1));
                }
            }
        }
