        ...
        if (!player.isCreative() && !player.isSpectator())
        {
            if (player.tickCount % 40 == 0 && !hasGrace)
            {
                ...
                if (bodyTemp >= 100 && !(hasFireResist && ConfigSettings.FIRE_RESISTANCE_ENABLED.get()))
                {
                    ...
                    this.dealTempDamage(player, ConfigSettings.DAMAGE_SCALING.get() ? hotScaling : hot, (float) CSMath.blend(ConfigSettings.TEMP_DAMAGE.get(), 0, heatResistance, 0, 1));
                }
                else if (bodyTemp <= -100 && !(hasIceResist && ConfigSettings.ICE_RESISTANCE_ENABLED.get()))
                {
                    ...
                    this.dealTempDamage(player, ConfigSettings.DAMAGE_SCALING.get() ? coldScaling : cold, (float) CSMath.blend(ConfigSettings.TEMP_DAMAGE.get(), 0, coldResistance, 0, 1));
                }
            }
        }
