...
import com.momosoftworks.coldsweat.common.entity.goal.EdibleTemptGoal;
...
public class ChameleonEntity extends AnimalEntity
{
    ...
    static final DataParameter<Integer> SHED_TIME = EntityDataManager.defineId(ChameleonEntity.class, DataSerializers.INT);
    ...
    {   this.goalSelector.addGoal(0, new SwimGoal(this));
        ...
        this.goalSelector.addGoal(4, new EdibleTemptGoal(this, 1.25, false));
    ...
    {
        if (entity instanceof ChameleonEntity && ((ChameleonEntity) entity).canShed())
        {
            ChameleonEntity chameleon = (ChameleonEntity) entity;
            if (chameleon.isShedding() && chameleon.canShed())
            {
                chameleon.shedItems();
    ...
    {
        if (event.getEntity().isAddedToWorld() && event.getEntity() instanceof ChameleonEntity)
        {
    ...
    public void tick()
    {
        ...
        if (!this.level.isClientSide)
        {
            ...
            {   this.stopShedding();
                ...
                WorldHelper.spawnParticleBatch(this.level, new ItemParticleData(ParticleTypes.ITEM, ModItems.CHAMELEON_MOLT.getDefaultInstance()),
        ...
        else if (this.random.nextDouble() < 0.2 && this.canShed())
        {
            ...
            WorldHelper.spawnParticle(this.level, new ItemParticleData(ParticleTypes.ITEM, ModItems.CHAMELEON_MOLT.getDefaultInstance()),
        ...
        if (this.tickCount % 5 == 0 && this.isTracking())
        {
            ...
            if (this.tickCount % 20 == 0)
            {
                ...
                && this.getServer() != null)
                {
                    ...
                    for (ServerPlayerEntity player : WorldHelper.getEntitiesOfClass(ServerPlayerEntity.class, this.level, this.getBoundingBox().inflate(20), e -> true))
                    {
                        if (advancement != null)
                        {
