...
public class ModRegistries
{
    ...
    // Special registries
    public static final RegistryHolder<RemoveRegistryData<?>> REMOVE_REGISTRY_DATA = createRegistry(new ResourceLocation(ColdSweat.MOD_ID, "remove"), RemoveRegistryData.CODEC, (Class) RemoveRegistryData.class);

    public static <V extends ConfigData> RegistryHolder<V> createRegistry(ResourceLocation registry, Codec<V> codec, Class<V> type)
    {
        RegistryHolder<V> registryHolder = new RegistryHolder<>(RegistryKey.createRegistryKey(registry), codec, type);
    ...
    public static RegistryKey<Registry<? extends ConfigData>> getRegistry(ResourceLocation name)
    {
