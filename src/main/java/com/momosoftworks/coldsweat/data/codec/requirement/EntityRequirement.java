                                ...
                                Optional<Predicate<Entity>> predicate)
{
    ...
    public static final Codec<EntityRequirement> SIMPLE_CODEC = RecordCodecBuilder.create(instance -> instance.group(
            NegatableList.listCodec(ConfigHelper.tagOrBuiltinCodec(Registries.ENTITY_TYPE, BuiltInRegistries.ENTITY_TYPE)).optionalFieldOf("entities", new NegatableList<>()).forGetter(requirement -> requirement.entities),
    ...
    public boolean test(Entity entity)
    {
        ...
        if (!target.isEmpty())
        {
            if (!(entity instanceof Monster monster) || !target.test(req -> req.test(monster.getTarget())))
            {   return false;
            }
        }
        if (!team.isEmpty())
