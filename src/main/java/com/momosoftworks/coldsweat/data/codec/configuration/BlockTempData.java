...
public class BlockTempData extends ConfigData
{
    ...
    final Optional<List<RegistryKey<BlockTempData>>> effectGroup;
    ...
    public BlockTempData(NegatableList<BlockRequirement> block, ValueGetter<Double> temperature, double range,
                         ...
                         NegatableList<EntityRequirement> entity, boolean logarithmic, Optional<List<RegistryKey<BlockTempData>>> effectGroup, NegatableList<String> requiredMods)
    {
        super(requiredMods);
        this.block = block;
    ...
    public BlockTempData(NegatableList<BlockRequirement> block, ValueGetter<Double> temperature, double range,
                         double maxEffect, boolean fade, WorldTempRequirement maxTemp, WorldTempRequirement minTemp,
                         Temperature.Units units, NegatableList<LocationRequirement> location,
                         NegatableList<EntityRequirement> entity, boolean logarithmic, Optional<List<RegistryKey<BlockTempData>>> effectGroup)
    {
    ...
    {
        this(new NegatableList<>(new BlockRequirement(blockTemp.getAffectedBlocks().stream().map(Either::<ITag<Block>, Block>right).collect(Collectors.toList()))),
    ...
    public static final Codec<BlockTempData> CODEC = createCodec(RecordCodecBuilder.mapCodec(instance -> instance.group(
            ...
            ExtraCodecs.deferred(() -> ExtraCodecs.registry(ModRegistries.BLOCK_TEMP_DATA.key())).listOf().optionalFieldOf("effect_group").forGetter(BlockTempData::effectGroup)
    ...
    public Optional<List<RegistryKey<BlockTempData>>> effectGroup()
    {   return effectGroup;
    }

    public double getTemperature(TileEntity blockEntity, BlockState state, Entity entity)
    {
    ...
    public static BlockTempData fromToml(List<?> entry)
    {
        ...
        // Get block predicate
        BlockRequirement.StateRequirement blockPredicates = entry.size() > 5 && entry.get(5) instanceof String && !NBTHelper.isBlank((String) entry.get(5))
                                                            ? BlockRequirement.StateRequirement.fromToml(((String) entry.get(5)).split(","), effectBlocks[0])
                                                            : BlockRequirement.StateRequirement.NONE;

        NbtRequirement nbtRequirement = entry.size() > 6 && entry.get(6) instanceof String && !NBTHelper.isBlank((String) entry.get(6))
                                        ? new NbtRequirement(NBTHelper.parseCompoundNbt((String) entry.get(6)))
                                        : NbtRequirement.NONE;

        boolean logarithmic = entry.size() > 8 && entry.get(8) instanceof Boolean && ((Boolean) entry.get(8));

        double maxTemperature = Double.POSITIVE_INFINITY;
        double minTemperature = Double.NEGATIVE_INFINITY;
        if (entry.size() > 7 && entry.get(7) instanceof Number)
        {
            Number tempLimit = (Number) entry.get(7);
