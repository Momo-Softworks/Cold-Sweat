...
public class LocationRequirement
{
    ...
    public boolean test(World level, int x, int y, int z)
    {
        ...
        && !level.getBiome(new BlockPos(x, y, z)).equals(this.biome.get()))
        {   return false;
        }

        if (this.structure.isPresent()
        && WorldHelper.getServerLevel(level).structureFeatureManager().getStructureAt(new BlockPos(x, y, z), false,
