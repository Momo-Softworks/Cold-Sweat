            ...
            for (String key : toMerge.getAllKeys())
            {
                ...
                if (originalValue != null)
                {
                    ...
                    else if (originalValue instanceof NumericTag originalNumber && toMergeValue instanceof StringTag operation && operation.getAsString().length() > 2)
                    {
                        ...
                        {
                            case ByteTag byteTag -> merged.putByte(key, (byte) numberValue);
                            case ShortTag shortTag -> merged.putShort(key, (short) numberValue);
                            case IntTag intTag -> merged.putInt(key, (int) numberValue);
                            case LongTag longTag -> merged.putLong(key, (long) numberValue);
                            case FloatTag floatTag -> merged.putFloat(key, (float) numberValue);
                            case DoubleTag doubleTag -> merged.putDouble(key, numberValue);
                            default -> {}
                        }
                    }
                }
