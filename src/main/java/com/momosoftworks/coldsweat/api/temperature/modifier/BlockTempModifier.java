...
public class BlockTempModifier extends TempModifier
{
    ...
    Map<Long, IChunk> chunks = new LinkedHashMap<>(16, 0.75f, true);
    Map<BlockTemp, BlockEffectData> blockTempTotals = new HashMap<>(16);
    Map<List<RegistryKey<BlockTempData>>, Double> groupTotals = new HashMap<>(8);
    ...
    public Function<Double, Double> calculate(LivingEntity entity, Temperature.Trait trait)
    {
        ...
        if (entity instanceof ServerPlayerEntity && shouldTickAdvancements)
        {
            ...
            {   ModAdvancementTriggers.BLOCK_AFFECTS_TEMP.trigger(((ServerPlayerEntity) entity), trigger.getA(), trigger.getC(), blockTempTotals.get(trigger.getB()).getTotalEffect());
            }
        }
    ...
    {
        if (blockTemp instanceof ConfiguredBlockTemp && ((ConfiguredBlockTemp) blockTemp).getData().effectGroup().isPresent())
        {   groupTotals.merge(((ConfiguredBlockTemp) blockTemp).getData().effectGroup().get(), delta, Double::sum);
        }
    }
    ...
    protected static class BlockEffectData
    {
        ...
        public BlockEffectData(LivingEntity entity, BlockTemp blockTemp, World level, BlockPos pos, BlockState state)
        {
