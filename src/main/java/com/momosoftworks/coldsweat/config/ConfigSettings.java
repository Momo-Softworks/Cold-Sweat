...
public class ConfigSettings
{
    ...
    public static final DynamicHolder<Integer> CHAMELEON_SHED_TIME_LIMIT;
    ...
    static
    {
        ...
        CHAMELEON_SHED_TIME_LIMIT = addSyncedSetting(ColdSweat.createKey("chameleon_shed_time_limit"), () -> 9600, holder -> holder.set(EntitySettingsConfig.CHAMELEON_SHED_TIME_LIMIT.get()),
        Codec.INT,
        (saver) -> EntitySettingsConfig.CHAMELEON_SHED_TIME_LIMIT.set(saver),
        SyncType.BOTH_WAYS);
        ...
        INSULATED_MOUNTS = addSetting(ColdSweat.createKey("insulated_entities"), RegistryMultiMap::new, holder ->
        {
            Multimap<EntityType<?>, MountData> dataMap = ConfigHelper.parseTomlRegistry(EntitySettingsConfig.INSULATED_MOUNTS,
                                                                                        MountData::fromToml,
                                                                                        data -> data.entity().flatten(d -> d.entities()),
        ...
        DISABLED_MODIFIERS = addSetting(ColdSweat.createKey("disabled_modifiers"), ArrayList::new, holder -> holder.get().addAll(MainSettingsConfig.DISABLED_TEMP_MODIFIERS.get().stream().map(ResourceLocation::new).collect(Collectors.toList())));
        ...
        WATER_DROPLET_SCALE = addClientSetting(ColdSweat.createKey("water_droplet_scale"), () -> new IntegerBounds(40, 48), holder -> holder.set(new IntegerBounds(ClientSettingsConfig.WATER_DROPLET_SCALE.get().toArray(new Integer[0]))),
            (saver) -> ClientSettingsConfig.WATER_DROPLET_SCALE.set(Arrays.asList(saver.min(), saver.max())));
    ...
    {
        SUPER_EASY("super_easy", () -> CSMath.mapOf(
