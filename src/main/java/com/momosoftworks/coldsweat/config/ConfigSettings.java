...
public class ConfigSettings
{
    ...
    static
    {
        ...
        (saver) -> ConfigHelper.writeItemMultimap(saver,
                                             ...
                                             fuel -> List.of(fuel.value(), fuel.data().components().write())));
        ...
        CARRIED_ITEM_TEMPERATURES = addSyncedSetting("carried_item_temps", new FastMultiMap<>(), holder ->
        {
            ...
            for (Object entry : list)
            {
                ...
                Either<TagKey<Item>, Item> item = itemID.startsWith("#")
                                                  ? Either.left(TagKey.create(Registries.ITEM, ResourceLocation.parse(itemID.substring(1))))
                                                  : Either.right(BuiltInRegistries.ITEM.get(ResourceLocation.parse(itemID)));
                //temp
                ...
                {
                    BuiltInRegistries.ITEM.getTag(item.left().get()).ifPresent(tag ->
                    {   tag.forEach(i -> map.put(i.value(), carriedTemp));
                    });
                }
                else
        ...
        (saver) -> WorldSettingsConfig.getInstance().setHearthSpreadWhitelist(saver.stream().map(block -> BuiltInRegistries.BLOCK.getKey(block)).toList()));
        ...
        (saver) -> WorldSettingsConfig.getInstance().setHearthSpreadBlacklist(saver.stream().map(block -> BuiltInRegistries.BLOCK.getKey(block)).toList()));

        HEARTH_STRENGTH = addSetting("hearth_effect", 0.75, holder -> holder.set(WorldSettingsConfig.getInstance().getHearthEffect()));
        ...
        BODY_ICON_POS = addClientSetting("body_icon_pos", new Vector2i(0, 0), holder -> holder.set(new Vector2i(ClientSettingsConfig.getInstance().getBodyIconX(),
        ...
        BODY_READOUT_POS = addClientSetting("body_readout_pos", new Vector2i(0, 0), holder -> holder.set(new Vector2i(ClientSettingsConfig.getInstance().getBodyReadoutX(),
        ...
        WORLD_GAUGE_POS = addClientSetting("world_gauge_pos", new Vector2i(0, 0), holder -> holder.set(new Vector2i(ClientSettingsConfig.getInstance().getWorldGaugeX(),
        ...
        CONFIG_BUTTON_POS = addClientSetting("config_button_pos", new Vector2i(0, 0), holder -> holder.set(new Vector2i(ClientSettingsConfig.getInstance().getConfigButtonPos().get(0),
