...
public class DepthTempModifier extends TempModifier
{
    ...
    public double getResult(Temperature temp, PlayerEntity player)
    {
        ...
        double depth = Math.max(0, WorldInfo.getGroundLevel(player.getPosition(), player.world) - player.getPosition().getY());
        ...
        valueMap.put(CSMath.blend(temp.get(), midTemp, !WorldInfo.canSeeSky(player.world, player.getPosition()) ? depth : depth / 2, 2, 20), 4.0);
